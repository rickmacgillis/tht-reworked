        ...
        public function content() { # Displays the page 
                ...
                switch($main->getvar['sub']) {
                        default:
                                if($_POST) {
                                        ...
                                        if(!$n) {
                                                ...
                                                elseif($db->num_rows($query) >= 1) {
                                                        ...
                                                        $db->query("INSERT INTO `<PRE>staff` (user, name, email, password, salt, perms, tzadjust) VALUES('{$main->postvar['user']}', '{$main->postvar['name']}', '{$main->postvar['email']}','{$password}','{$salt}', '{$string}', '{$main->postvar['tzones']}')");
                                                        $main->errors("Account added!");        
                                                }
                                        }
                                }
                                $query = $db->query("SELECT * FROM `<PRE>acpnav` WHERE link != 'home'");
                                ...
                                $array['PAGES'] .= '<tr><td width="30%" align="left">Paid Configuration:</td><td><input name="pages_paid" id="pages_paid" type="checkbox" value="1" /></td></tr>';
                                $array['PAGES'] .= '<tr><td width="30%" align="left">P2H Forums:</td><td><input name="pages_p2h" id="pages_p2h" type="checkbox" value="1" /></td></tr>';
                                $array['PAGES'] .= "</table>";
                                $array['TZADJUST'] = $main->tzlist();
                        ...
                        case "edit":
                                if(isset($main->getvar['do'])) {
                                        ...
                                        if($db->num_rows($query) == 0) {
                                                ...
                                                if($_POST) {
                                                        ...
                                                        if(!$n) {
                                                                if(!$main->check_email($main->postvar['email'])) {
                                                                        ...
                                                                        $db->query("UPDATE `<PRE>staff` SET `tzadjust` = '{$main->postvar['tzones']}' WHERE `user` = '{$main->getvar['do']}'");
                                                ...
                                                if(substr_count($data['perms'], "paid") == '1'){
                                                $paid_check = 'checked="checked"';
                                                }
                                                if(substr_count($data['perms'], "p2h") == '1'){
                                                $p2h_check = 'checked="checked"';
                                                }
                                                $array['PAGES'] .= '<tr><td width="30%" align="left">Paid Configuration:</td><td><input name="pages_paid" id="pages_paid" type="checkbox" value="1" '.$paid_check.'/></td></tr>'."\n";
                                                $array['PAGES'] .= '<tr><td width="30%" align="left">P2H Forums:</td><td><input name="pages_p2h" id="pages_p2h" type="checkbox" value="1" '.$p2h_check.'/></td></tr>'."\n";
                                                $array['PAGES'] .= "</table>";
                        ...
                        case "delete":
                                ...
                                $query = $db->query("SELECT * FROM `<PRE>staff`");  //This pulls the current staff list after deletion.
                                        echo "<ERRORS>";
                                        while($data = $db->fetch_array($query)) {
