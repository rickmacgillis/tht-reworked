        ...
        public function content() { # Displays the page 
                ...
                if($_POST) {
                        foreach($main->postvar as $key => $value) {
                                if($value == "" && !$n && $key != "alerts" && $key != "akismetkey" && $key != "otherdefault") {
                                        $main->errors("Please fill in all the fields!");
                                        $n++;
                                }
                                
                                if($main->postvar['url'] && substr($main->postvar['url'], -1, 1) != "/"){
                                      $main->postvar['url'] = $main->postvar['url']."/";
                                }
                                
                                if($main->postvar['default'] && $main->postvar['default'] == "-other-"){
                                  if(!$main->postvar['otherdefault']){
                                      $main->errors("Please enter the default directory to redirect to.");
                                      $n = 1;
                                  }else{
                                      if(is_dir("../".$main->postvar['otherdefault'])){
                                      $main->postvar['default'] = $main->postvar['otherdefault'];
                                      }else{
                ...
                switch($main->getvar['sub']) {
                        default:
                                ...
                                if($db->config("default") != ADMINDIR && $db->config("default") != "order" && $db->config("default") != "client"){
                                $array['DROPDOWN'] = $main->dropDown("default", $values, "-other-");
                                $array['OTHERDEFAULT'] = $db->config("default");
                                }else{
                                $array['OTHERDEFAULT'] = "";
                        ...
                        case "security": #security settings
                            ...
                            $array['EMAIL_ON_CRON'] = $main->dropDown("emailoncron", $values, $db->config("emailoncron"));
                        ...
                        case "signup":
                                ...
                                $array['USEAKISMET'] = $main->dropDown("useakismet", $values, $db->config("useakismet"));
                                $array['AKISMETKEY'] = $db->config("akismetkey");
