                ...
                public function acpPage() {
                        ...
                        if($_POST) {
                                foreach($main->postvar as $key => $value) {
                                        if($value == "" && !$n && $key != "password" && $key != "paypalsandemail") {
                                ...
                                if(!$n) {
                                        if(is_numeric($main->postvar['susdays']) && is_numeric($main->postvar['termdays'])) {
                                                ...
                                                $db->updateConfig("paypalmode", $main->postvar['paypalmode']);
                                                $db->updateConfig("paypalsandemail", $main->postvar['paypalsandemail']);
                        ...
                        $ppvalues[] = array("Live","live");
                        $ppvalues[] = array("Sandbox","sandbox");
                        
                        $array['CURRENCY'] = $main->dropDown("currency", $values, $db->config("currency"));
                        $array['PAYPALMODE'] = $main->dropDown("paypalmode", $ppvalues, $db->config("paypalmode"));
                        $array['PAYPALSANDEMAIL'] = $db->config("paypalsandemail");
